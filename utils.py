...
def craw_setn(n=10):
    ...
    soup = BeautifulSoup(
        requests.get("https://www.setn.com/Catalog.aspx?PageGroupID=6").text, "lxml"
    )
    for s in tqdm(soup.find_all("a", pl="熱門新聞")[:n]):
        data["titles"].append(s.text)
        data["urls"].append("https://www.setn.com" + s["href"])
        data["contents"].append(
            BeautifulSoup(requests.get(data["urls"][-1]).text, "lxml")
            .find_all("article")[0]
            .text
        )
        time.sleep(1)
...
def craw_ctinews(n=5, sleep_time=3):
    ...
    for s in soup.find_all("div", class_="base-card__news-title")[:n]:
        try:
            ...
            urls.append(
                "https://ctinews.com" + re.search('href="(.+?)"', str(s)).group(1)
            )
            contents.append(
                BeautifulSoup(requests.get(urls[-1]).text, "html.parser")
                .find_all(
                    "div",
                    class_="rendered-content em:text-xl leading-normal ck-content",
                )[0]
                .text
            )
    ...
    if r.status_code == requests.codes.ok:
        ...
        for s in tqdm(stories[:n]):
            if type(s.get("aria-label")) is str:
                ...
                contents.append(
                    preprocess_raw_sentence(
                        str(
                            BeautifulSoup(
                                requests.get(s.get("href")).text, "html.parser"
                            ).find_all("section", class_="article-content__editor")
                        )
                    )
                )
    df["title"] = titles
...
# 爬ettoday新聞網(因需要selenium比較費時故先棄用)
"""
...
#TODO: crawler
# 爬ptt政治板
def get_some_page_ptt_data(
    URL="https://www.ptt.cc/bbs/HatePolitics/index.html", PTT_n_page=10
):
    ...
    def get_this_index_data(URL="https://www.ptt.cc/bbs/HatePolitics/index.html"):
        ...
        for i in result:
            try:
                post_url = (
                    "https://www.ptt.cc" + str(i).split('href="')[1].split('">')[0]
                )
    ...
    URLS = [
        f"https://www.ptt.cc/bbs/HatePolitics/index{i}.html"
        for i in range(total_page_number, total_page_number - PTT_n_page, -1)
    ]
...
def get_score_by_person(
    ...
    df = get_some_page_ptt_data(
        URL="https://www.ptt.cc/bbs/HatePolitics/index.html", PTT_n_page=PTT_n_page
    )
    # 判斷是否保存資料
    if save["ptt"] == True:
